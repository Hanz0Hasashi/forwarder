...
export async function GET() {
    try {
        ...
        if (!response.ok) {
            return json({ error: 'Failed to fetch users' }, { status: response.status });
        }

        const clerkUsers = await response.json();

        // Normalize Clerk user shape to keep frontend rendering simple.
...
export async function PATCH({ request }: RequestEvent) {
    ...
    if (!response.ok) {
        return json({ error: 'Failed to update role' }, { status: response.status });
    }
...
export async function DELETE({ request }: RequestEvent) {
    ...
    if (!response.ok) {
        return json({ error: 'Failed to delete user' }, { status: response.status });
    }
